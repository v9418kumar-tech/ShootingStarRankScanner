...
def load_instruments():
    ...
    for x in data:
        if x.get("segment")=="NSE_EQ" and x.get("instrument_type")=="EQ":
            k=x.get("instrument_key")
            s=(x.get("trading_symbol") or "").strip().upper()
            if k and s:
...
def quote_batch(batch):
    try:
        ...
        r=requests.get(QUOTE_URL,headers=hdr(),
                       params={"instrument_key":keys},timeout=20)
...
def ss_score(c,prev):
    try:
        ts,o,h,l,cl,vol=c[:6]
        o,h,l,cl,vol=map(float,(o,h,l,cl,vol))
    ...
    if len(prev)>=4:
        try:
            ...
            if a>0:trend=clamp((b-a)/a/.008*100)
        except:pass
    ...
    # ---------------------------------------------------------
    # FINAL ACTUAL CONDITIONS
    # 1. Shooting Star candle RED
    # 2. Immediately previous candle GREEN
    # 3. Shooting Star volume > previous green candle volume
    # 4. Strong upper wick
    # 5. Small lower wick
    # 6. Body near lower part
    # 7. Close near low
    # 8. Previous uptrend
    # ---------------------------------------------------------
    ...
    if prev:
        try:
            ...
            pc=sf(prev[-1][4])
            pv=sf(prev[-1][5])
            previous_green=pc>po
            volume_confirmation=vol>pv
        except:pass
...
def scan():
    ...
    if not TOKEN:
        raise RuntimeError("UPSTOX_ACCESS_TOKEN Render Environment Variable में नहीं मिला।")

    load_instruments()
    quotes=live_quotes()

    if not quotes:
        raise RuntimeError("Upstox से live quotes नहीं मिले।")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for f in as_completed(fs):
            try:
                ...
                if dev:
                    developing.append(dev)

            except Exception as e:
                print("Analysis:",e)

    rows=list(TODAY_ACTUAL.values())+developing
    rows.sort(key=lambda x:x["score"],reverse=True)

    final=[]

    for rank,x in enumerate(rows[:MAX_DISPLAY],1):
        final.append({
            "rank":rank,
            "symbol":x["symbol"],
...
def market_open():
    ...
    a=datetime.strptime("09:15","%H:%M").time()
    b=datetime.strptime("15:35","%H:%M").time()
    return a<=t<=b

def background():
    while True:
        try:
            if market_open() and not SCAN_RUNNING:
                start_scan()
        except Exception as e:
            print("Background:",e)
...
body{margin:0;padding:6px;background:#11151a;color:#e8edf3;font-family:Arial}
.card{background:#1a2027;border:1px solid #303944;border-radius:9px;padding:8px;margin-bottom:8px}
h1{font-size:20px;margin:2px 0 3px}
.sub,.status,.note{color:#9fa8b3;font-size:12px}
button{background:#2864e8;color:#fff;border:0;border-radius:7px;padding:9px 14px;font-size:14px;font-weight:bold}
.status{margin-top:7px}.wrap{overflow-x:auto}
table{width:100%;min-width:650px;border-collapse:collapse;font-size:12px}
th{background:#252c35;padding:7px 4px;white-space:nowrap}
td{padding:7px 4px;border-bottom:1px solid #2c333c;text-align:center;white-space:nowrap}
.actual{background:#123529}.developing{background:#302711}
.score{font-weight:bold;font-size:14px}.note{line-height:1.5}
</style></head><body>
...
<div id="status" class="status">Scanner ready</div>
</div>

<div class="card">
<b>⭐ SHOOTING STAR RESULTS — TODAY</b>
<div class="wrap"><table>
<thead><tr>
<th>Rank</th><th>Share</th><th>Price</th><th>Chg%</th>
<th>SS Time</th><th>Score</th><th>Status</th>
</tr></thead>
<tbody id="rows"></tbody>
</table></div></div>
...
<b>Actual Shooting Star:</b><br>
• Completed 5-minute candle<br>
• Candle लाल होनी चाहिए<br>
• उससे पिछली candle हरी होनी चाहिए<br>
• लाल candle का Volume पिछली हरी candle से अधिक होना चाहिए<br>
• Long upper wick + small body<br>
• Small lower wick<br>
• Close low के पास<br>
• Previous uptrend<br><br>

<b>Developing</b> = वर्तमान चल रही 5-minute candle की structure।<br>
...
function draw(rows){
 ...
 if(!rows||!rows.length){
  b.innerHTML="<tr><td colspan='7'>अभी कोई qualifying result नहीं मिला</td></tr>";
  return;
 }
 rows.forEach(x=>{
  const tr=document.createElement("tr");
  tr.className=x.status=="Actual"?"actual":"developing";
  tr.innerHTML="<td><b>"+x.rank+"</b></td>"+
  "<td><b>"+x.symbol+"</b></td>"+
  "<td>₹"+Number(x.price).toFixed(2)+"</td>"+
  "<td>"+Number(x.change).toFixed(2)+"%</td>"+
  "<td>"+x.time+"</td>"+
  "<td class='score'>"+Number(x.score).toFixed(1)+"</td>"+
  "<td>"+x.status+"</td>";
  b.appendChild(tr);
 });
}
...
async function load(){
 try{
  ...
  document.getElementById("status").innerText="Connection problem";
 }
}
...
@app.route("/api/start")
def api_start():
    return jsonify({"started":start_scan(),"running":SCAN_RUNNING})
